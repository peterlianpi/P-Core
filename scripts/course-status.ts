import { CourseStatus, PrismaClient } from "@/prisma-features-database/features-database-client-types";
...
async function seedCourseStatusLogs() {
    ...
    for (const sc of studentCourses) {
        ...
        for (let i = 0; i < numberOfChanges; i++) {
            ...
            const statuses = possibleStatuses.filter((s) => s !== CourseStatus.ENROLLED);
