...
const getRandomItem = <T extends readonly unknown[]>(arr: T): T[number] => {
...
export const mockCourses: Course[] = COURSE_NAMES.map((name, i) => ({
  ...
  levels: COURSE_LEVELS.slice(
    0,
    Math.floor(Math.random() * COURSE_LEVELS.length) + 1
  ),
  monthlyFee: 100 + i * 20,
  isActive: true,
  createdAt: now,
...
// Generate Students
export const mockStudents: Student[] = Array.from({ length: 50 }).map(
  (_, i) => {
...
export const mockTeachers: Teacher[] = Array.from({ length: 50 }).map(
  (_, i) => ({
    id: (i + 1).toString(),
    name: `Teacher ${i + 1}`,
    ...
    avatar: "/placeholder-user.jpg",
    specialization: Array.from({ length: 2 }).map(() =>
      getRandomItem(SPECIALIZATIONS)
    ),
    isActive: true,
    createdAt: now,
    updatedAt: now,
...
export const mockLessonBooks: LessonBook[] = Array.from({ length: 50 }).map(
  (_, i) => {
    const course = getRandomItem(mockCourses);
    return {
      id: (i + 1).toString(),
...
export const mockDashboardStats: DashboardStats = {
  ...
  pendingPayments: mockTransactions.filter((tx) => tx.status === "pending")
    .length,
