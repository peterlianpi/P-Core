...
const studentCourses = new Hono()
  ...
  .get(
    ...
    async (c) => {
      ...
      const records = await featuresDBPrismaClient.studentCourse.findMany({
        where: q,
      });
  ...
  // GET one
  .get("/:id", async (c) => {
    const rec = await featuresDBPrismaClient.studentCourse.findUnique({
  ...
  .patch(
    "/:id",
    zValidator("json", z.object({ status: enrollmentSchema.shape.status })),
    async (c) => {
      const { status } = c.req.valid("json");
      const rec = await featuresDBPrismaClient.studentCourse.update({
        where: { id: c.req.param("id") },
        data: { status },
      });
      return c.json(rec);
