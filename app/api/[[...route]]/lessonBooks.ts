...
const lessonBooks = new Hono()
  ...
  .get(
    ...
    async (c) => {
      ...
      const books = await featuresDBPrismaClient.lessonBook.findMany({
        where: { courseId },
      });
      return c.json(books);
    }
  ...
  // GET one
  .get("/:id", async (c) => {
    const book = await featuresDBPrismaClient.lessonBook.findUnique({
  ...
  // UPDATE
  .patch("/:id", zValidator("json", bookSchema.partial()), async (c) => {
    const data = c.req.valid("json");
  ...
  // DELETE
  .delete("/:id", async (c) => {
    await featuresDBPrismaClient.lessonBook.delete({
      where: { id: c.req.param("id") },
    });
