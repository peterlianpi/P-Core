    ...
    async (c) => {
      try {
        ...
        const [books, totalCount, activeCount, inactiveCount, availableCount, loanedCount] = await Promise.all([
          ...
          prisma.book.count({ where: { isActive: true, available: { lt: prisma.book.fields.copies } } })
        ]);
        ...
        return c.json({
          ...
          inventoryStats: {
            totalCopies: inventoryStats._sum.copies || 0,
            availableCopies: inventoryStats._sum.available || 0,
            loanedCopies: (inventoryStats._sum.copies || 0) - (inventoryStats._sum.available || 0),
