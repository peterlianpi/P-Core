...
const courseStatusLogs = new Hono()
  ...
  .get(
    "/",
    zValidator("query", z.object({ studentCourseId: z.string().optional() })),
    async (c) => {
      const q = c.req.valid("query");
      const logs = await featuresDBPrismaClient.courseStatusLog.findMany({
        where: q,
  ...
  // GET one log
  .get("/:id", async (c) => {
    const log = await featuresDBPrismaClient.courseStatusLog.findUnique({
  ...
  // CREATE log
  .post("/", zValidator("json", logSchema), async (c) => {
  ...
  // DELETE log
  .delete("/:id", async (c) => {
    await featuresDBPrismaClient.courseStatusLog.delete({
      where: { id: c.req.param("id") },
    });
